...
const CheckoutPage = props => {
  ...
  // Fetch the seller's Stripe Connect account id so the Payment Request Button can
  // initialise the wallet sheet with the matching `onBehalfOf` (Butterfly customization).
  const onFetchProviderStripeAccountId = useCallback(
    listingId => dispatch(fetchProviderStripeAccountId(listingId)),
    [dispatch]
  );
